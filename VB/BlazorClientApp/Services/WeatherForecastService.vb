...
Namespace BlazorClientApp.Services
    ...
    Public Class WeatherForecastService
        ...
        Private temperatureStatistic As Tuple(Of Integer, Integer)() = New Tuple(Of Integer, Integer)() {Tuple.Create(-15, -1), Tuple.Create(-12, -2), Tuple.Create(-6, 8), Tuple.Create(4, 14), Tuple.Create(11, 21), Tuple.Create(13, 23), Tuple.Create(18, 32), Tuple.Create(16, 26), Tuple.Create(10, 18), Tuple.Create(1, 11), Tuple.Create(-4, 6), Tuple.Create(-13, 1)}

        Private ConditionsForSummary As Tuple(Of Integer, String)() = New Tuple(Of Integer, String)() {Tuple.Create(28, "Hot"), Tuple.Create(14, "Warm"), Tuple.Create(-9, "Cold"), Tuple.Create(-80, "Freezing")}
        ...
        Private Function CreateForecast() As List(Of WeatherForecast)
            ...
            Return Enumerable.Range(1, 15).[Select](Function(index)
                ...
                If day.Month - startDate.AddDays(CDbl(index - 1)).Month <> 0 OrElse temperatureC Is Nothing Then
                    min = temperatureStatistic(CInt(day.Month - 1)).Item1
                    max = temperatureStatistic(CInt(day.Month - 1)).Item2
                ...
                Dim weatherTypes As Integer = If(rng.NextDouble() < 0.5, 0, (If(rng.NextDouble() < 0.5, 1, If(rng.NextDouble() < 0.5, 2, 3))))
                Dim feelTemper As Double = temperatureC.Value - (weatherTypes - 1) * 3
                Return New WeatherForecast With {.[Date] = day, .TemperatureC = temperatureC.Value, .Precipitates = rng.NextDouble() < weatherTypes * 0.3, .WeatherType = WeatherForecastService.WeatherTypes(weatherTypes), .Summary = Enumerable.First(Of Tuple(Of Global.System.Int32, Global.System.[String]))(ConditionsForSummary, CType(Function(c) CBool(c.Item1 <= feelTemper), Func(Of Tuple(Of Integer, String), Boolean))).Item2}
            End Function).ToList()
        End Function
        ...
        Private Function CreateDetailedForecast() As List(Of WeatherForecast)
            ...
            Return Enumerable.Range(1, 15).SelectMany(Function(day)
                ...
                Return Enumerable.Range(0, 24).[Select](Function(hour) New WeatherForecast With {.[Date] = dayDate.AddHours(hour), .TemperatureC = rng.Next(minTemp, maxTemp), .Summary = ConditionsForSummary(rng.Next(ConditionsForSummary.Length)).Item2, .WeatherType = WeatherTypes(rng.Next(WeatherTypes.Length)), .Precipitates = rng.Next(100) < 30})
            End Function).ToList()
        End Function
        ...
        Public Function GetForecastAsync(ByVal Optional ct As CancellationToken = Nothing) As Task(Of IEnumerable(Of WeatherForecast))
            Return Task.FromResult(Forecasts.AsEnumerable())
        End Function

        Public Function GetDetailedForecastAsync(ByVal Optional ct As CancellationToken = Nothing) As Task(Of WeatherForecast())
            Return Task.FromResult(DetailedForecast.ToArray())
        End Function

        Public Function GetSummariesAsync(ByVal Optional ct As CancellationToken = Nothing) As Task(Of String())
            Return Task.FromResult(ConditionsForSummary.[Select](Function(v) v.Item2).ToArray())
        End Function

        Public Function GetCloudinessAsync(ByVal Optional ct As CancellationToken = Nothing) As Task(Of IEnumerable(Of String))
            ...
            For Each field In newValue.Keys
                Select Case field
                    Case NameOf(dataItem.Date)
                        dataItem.Date = CDate(newValue(field))
                    Case NameOf(dataItem.Summary)
                        dataItem.Summary = CStr(newValue(field))
                    Case NameOf(dataItem.TemperatureC)
                        dataItem.TemperatureC = CInt(newValue(field))
                    Case NameOf(dataItem.Precipitates)
                        dataItem.Precipitates = CBool(newValue(field))
                    Case NameOf(dataItem.WeatherType)
                        dataItem.WeatherType = CStr(newValue(field))
                End Select
            Next
